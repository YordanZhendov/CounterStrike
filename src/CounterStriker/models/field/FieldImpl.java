...
public class FieldImpl implements Field {
    ...
    public String start(Collection<Player> players) {
        Collection<Player> counterTerrorists = players.stream().filter(c -> c instanceof CounterTerrorist).collect(Collectors.toList());
        Collection<Player> terrorists=players.stream().filter(c-> c instanceof Terrorist).collect(Collectors.toList());
